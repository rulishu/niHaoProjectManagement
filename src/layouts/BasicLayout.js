...
function BasicLayoutScreen(props = {
  routes: []
}) {
  const layouts = useLayouts();
  const {
    mutate
  } = useSWR(['/api/reloadAuth', {
    method: 'POST'
  }], {
  ...
  const basicLayoutProps = {
    ...
    headerFontColor: '#fff' // hideReloadButton: true,
  ...
  return <AuthPage authority={true} redirectPath="/login">
      <BasicLayout {...basicLayoutProps} onLogoClick={event => {
