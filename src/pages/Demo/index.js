...
const Demo = () => {
  ...
  const menu = <div>
      ...
      }].map((item, idx) => <Menu.Item onClick={item?.onClick} key={idx} text={item.label} />)}
      </Menu>
    </div>;
  return <>
      <ProTable searchBtns={[{
      ...
      render: <Dropdown menu={menu} trigger="click" placement="bottomRight">
      ...
      key: 'edit',
      width: 98,
      render: (text, key, rowData) => <div style={{
        alignItems: 'center',
        justifyContent: 'center',
        display: 'flex'
      }}>
                <Button size="small" type="danger" onClick={handleEditTable.bind(this, 'edit', rowData)}>
                  编辑
                </Button>
                <Button size="small" type="success" onClick={handleEditTable.bind(this, 'view', rowData)}>
                  查看
                </Button>
              </div>
    }]} table={table} />
